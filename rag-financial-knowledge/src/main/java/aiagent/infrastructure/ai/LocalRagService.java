...
/**
 * Title Cloud rag service.<br> Description Cloud rag service.<br>
 *
 * @author yuanci.ytb
 * @since 1.0.0-M2
 */
...
public class LocalRagService implements RagService {
    ...
    public void importDocuments() {
        ...
        if (splitDocuments.isEmpty()) {
            logger.info("splitDocuments is empty");
            return;
        }
    ...
    public Flux<ChatResponse> retrieve(String message) {
        System.out.println("Elasticsearch Client version: " +
            ElasticsearchClient.class.getPackage().getImplementationVersion());


        // Enable hybrid search, both embedding and full text search
        SearchRequest searchRequest = SearchRequest.builder().topK(4).similarityThresholdAll()
            .filterExpression(new FilterExpressionBuilder().eq(textField, message).build()).build();
    ...
    private void createIndexIfNotExists() {
        try {
            ...
            IndexSettings indexSettings = IndexSettings.of(
                settings -> settings.numberOfShards(String.valueOf(1)).numberOfReplicas(String.valueOf(1)));
            ...
            CreateIndexResponse indexResponse = elasticsearchClient.indices().create(
                createIndexBuilder -> createIndexBuilder.index(indexName).settings(indexSettings)
